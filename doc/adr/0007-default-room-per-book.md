...
- The light picker starts filtered to that room and drops the room headings, which carry no information with one room. A line under the search switches between "Nur «Raum»" and "Alle Räume" for that one visit; the book's setting is changed only through the chip.
- A scene already chosen in a moment stays visible and selected regardless of the filter, as does a scene that is no longer on the bridge.
